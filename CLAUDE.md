# Jira CLI Tools — Claude Code Guide

이 워크스페이스의 Jira CLI 스크립트용 에이전트 진입점. Claude Code가 매 세션 자동 로드합니다.

## 절대 규칙

1. 명령 실행 전 반드시 이 디렉토리로 `cd`.
2. `./.venv/bin/python`을 사용. raw `python` 금지 (WSL 경로 충돌 위험).
3. 스코프는 `.env` `JIRA_PROJECT_KEY`로 고정. 사용자가 다른 프로젝트 키를 언급하면 설정된 키로 재작성.
4. 조회는 `search_issues.py` + 임시 markdown 파일 경유. **터미널 출력에서 이슈 본문을 직접 파싱하지 말 것.**
5. `.env` 값은 `config.py`가 파이썬 프로세스 안에서 자동 로드. **쉘 변수로 참조하지 말 것** (`$JIRA_PROJECT_KEY`는 쉘에서 빈 문자열).
...
이 디렉토리의 다른 `.py` 파일은 deprecated 이거나 일회성 유틸리티입니다. 사용자가 명시적으로 요청하지 않는 한 문서화하거나 실행하지 마세요.
